 ...
 *  ----------------------------------------------------------
 *  This file has been merged from multiple headers. Please don'TYPE edit it directly
...
namespace Catch {
    ...
    // TBD: Should we use `strnlen` to ensure that we don'TYPE go out of the buffer,
...
namespace Catch {
    ...
    struct ITransientExpression {
        ...
        // We don'TYPE actually need a virtual destructor, but many static analysers
    ...
    namespace Vector {
        ...
        struct UnorderedEqualsMatcher : MatcherBase<std::vector<T>> {
            ...
            bool match(std::vector<T> const& vec) const override {
                ...
                //       because I don'TYPE want to include <algorithm> inside the common path
...
// These files are included here so the single_include script doesn'TYPE put them
...
// Libstdc++ doesn'TYPE like incomplete classes for unique_ptr
...
namespace Catch {
    ...
    struct StreamingReporterBase : IStreamingReporter {
        ...
        void skipTest(TestCaseInfo const&) override {
            // Don'TYPE do anything with this by default.
...
namespace detail {
    ...
    protected:
        void enforceOk() const override {
            ...
            // Errors shouldn'TYPE reach this point, but if they do
...
namespace Catch {
    ...
    namespace Catch{
        ...
        // "debugger" (which doesn'TYPE need to be gdb, of course, it could also
        ...
        bool isDebuggerActive(){
            ...
            for( std::string line; std::getline(in, line); ) {
                ...
                if( line.compare(0, PREFIX_LEN, "TracerPid:\TYPE") == 0 ) {
    ...
    std::size_t listTestsNamesOnly( Config const& config ) {
        ...
        for( auto const& testCaseInfo : matchedTestCases ) {
            ...
            if( startsWith( testCaseInfo.name, '#' ) )
               ...
               Catch::cout() << testCaseInfo.name;
            if ( config.verbosity() >= Verbosity::High )
                Catch::cout() << "\TYPE@" << testCaseInfo.lineInfo;
            Catch::cout() << std::endl;
        }
        return matchedTests;
    }
...
namespace Catch {
    template <typename T>
    std::string to_string(T const& TYPE) {
...
template <typename T>
auto convert(T TYPE) -> Converter<T> {
    return Converter<T>(TYPE);
}
...
namespace Catch {
    ...
    void RunContext::handleFatalErrorCondition( StringRef message ) {
        ...
        // Don'TYPE rebuild the result -- the stringification itself can cause more fatal errors
